...
void NodesUsage::SortVector(std::vector<UsageStruct> &vector, std::string sort_by){
    ...
    if (sort_by == "txFlows")
    {
        std::sort(vector.begin(), vector.end(), less_txFlows());
    }
    else if (sort_by == "rxFlows")
    {
        std::sort(vector.begin(), vector.end(), less_rxFlows());
    }
    else if (sort_by == "txBytes")
    {
        std::sort(vector.begin(), vector.end(), less_txBytes());
    }
    else if (sort_by == "rxBytes")
    {
        std::sort(vector.begin(), vector.end(), less_rxBytes());
    }
}
